...
| **FR-08** (Undo Availability)| ADR-004 (Option A) | `Game.CanUndo`, `Game.Undo()`, `CannotUndoException` | `GameUndoTests.CanUndo_OnNewGame_ReturnsFalse`, `Undo_AfterGameWon_ThrowsCannotUndoExceptionAndPreservesTerminalState` | Unit Test | Domain portion: Implemented; Overall requirement: Partially Implemented (Deferred integration: API/UI) |
| **FR-09** (Two Player Undo) | ADR-004, Memento | `GameMemento`, `Game.Undo()`, `Game._undoStack` | `GameUndoTests.Undo_AfterSingleMove_RestoresInitialEmptyBoardAndPlayerX`, `Undo_AfterTwoMoves_RestoresFirstMoveAndPlayerOTurn` | Unit Test | Domain portion: Implemented; Overall requirement: Partially Implemented (Deferred integration: API/UI) |
| **FR-10** (Computer Undo) | ADR-004, Memento | `GameMemento`, `Game.MakeMove`, `Game.Undo()`, `Game.ExecuteTurn` | `ComputerModeIntegrationTests.Undo_AfterCompletedComputerTurn_RevertsBothHumanAndComputerMovesRestoringPlayerX` | Unit / Integration Test | Domain portion: Implemented and Verified; Overall requirement: Partially Implemented (Deferred integration: API/UI) |
| **FR-11** (Scoreboard) | ADR-007 | `Scoreboard` Aggregate, `IScoreboardRepository`, `InMemoryScoreboardRepository`, `GameCompletedEventHandler` | `ScoreboardTests`, `GameCompletedEventHandlerTests`, `EventLifecycleTests` | Unit / Integration Test | Domain and Application Complete; API and UI Presentation Deferred (P010/P011) |
| **FR-12** (Reset Game) | A-002, P008.2 | `Game.Reset()` (preserves `GameId`, retains `DomainEvents`) | `GameResetTests`, `ResetIndependenceTests`, `EventLifecycleTests` | Unit Test | Domain Aggregate Complete; API Endpoint and UI Action Deferred (P010/P011) |
| **FR-13** (Reset Scoreboard)| ADR-007, P008.2 | `Scoreboard.Reset()` (clears counters, preserves idempotency history) | `ScoreboardTests`, `ResetIndependenceTests` | Unit Test | Domain Aggregate Complete; API Endpoint and UI Action Deferred (P010/P011) |
| **FR-14** (Computer Strategy)| ADR-005, Strategy | `IComputerMoveStrategy`, `BasicComputerMoveStrategy`, `Game.PlayComputerMove` | `ComputerStrategyTests`, `ComputerModeIntegrationTests` | Unit / Integration Test | Domain portion: Implemented; Overall requirement: Partially Implemented (Deferred integration: API/UI) |
