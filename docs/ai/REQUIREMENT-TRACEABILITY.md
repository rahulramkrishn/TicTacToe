...
|---|---|---|---|---|---|
| **FR-01** (Create Game) | ADR-001, ADR-002 | `Game` Aggregate, `IGameService.CreateGameAsync`, `InMemoryGameRepository` | `Create_InitializesGameInStandardInitialState`, `GameServiceTests.CreateGameAsync` | Unit / Application Test | Domain & Application Complete; API/UI Deferred (P010/P011) |
| **FR-02** (Board 0..8) | A-001, ADR-001 | `Board` Entity, `CellIndex` Value Object, `GameStateDto` | `BoardTests`, `CellIndexTests`, `GameServiceTests` | Unit / Application Test | Domain & Application Complete; UI Rendering Deferred (P011) |
| **FR-03** (Turns) | ADR-001, ADR-003 | `Game.MakeMove()`, `IGameService.MakeMoveAsync` | `MakeMove_ValidMove_PlacesMarkAppendsHistoryAndAlternatesTurn`, `GameServiceTests` | Unit / Application Test | Domain & Application Complete; API/UI Deferred (P010/P011) |
| **FR-04** (Win Detection) | ADR-001, ADR-007 | `WinDetector` Domain Service, `Game.MakeMove`, `IGameService.MakeMoveAsync` | `WinDetectorTests`, `GameServiceTests.MakeMoveAsync_TerminalMove_DispatchesAndClearsDomainEvents` | Unit / Application Test | Domain & Application Complete; API/UI Deferred (P010/P011) |
| **FR-05** (Draw Detection) | ADR-001, ADR-007 | `WinDetector` Domain Service, `Game.MakeMove`, `IGameService.MakeMoveAsync` | `MakeMove_StandardDraw`, `GameServiceTests` | Unit / Application Test | Domain & Application Complete; API/UI Deferred (P010/P011) |
| **FR-06** (Move Validation) | ADR-001, ADR-003 | `Game.MakeMove()` invariants, Domain Exceptions, `IGameService` | `MakeMove_OccupiedCell`, `GameServiceTests.MakeMoveAsync_OccupiedCell_ThrowsCellOccupiedException` | Unit / Application Test | Domain & Application Complete; HTTP ProblemDetails / UI Disabling Deferred (P010/P011) |
| **FR-07** (Move History) | A-001, ADR-001 | `Move` Value Object, `Game.MoveHistory`, `MoveDto` | `MakeMove_SequentialValidMoves`, `GameServiceTests.MakeMoveAsync` | Unit / Application Test | Domain & Application Complete; API/UI Presentation Deferred (P010/P011) |
| **FR-08** (Undo Availability)| ADR-004 (Option A) | `Game.CanUndo`, `Game.Undo()`, `IGameService.UndoAsync` | `GameUndoTests`, `GameServiceTests.UndoAsync_TwoPlayerMode_UndoesSingleMove` | Unit / Application Test | Domain & Application Complete; API/UI Deferred (P010/P011) |
| **FR-09** (Two Player Undo) | ADR-004, Memento | `GameMemento`, `Game.Undo()`, `IGameService.UndoAsync` | `GameUndoTests`, `GameServiceTests.UndoAsync_TwoPlayerMode_UndoesSingleMove` | Unit / Application Test | Domain & Application Complete; API/UI Deferred (P010/P011) |
| **FR-10** (Computer Undo) | ADR-004, Memento | `GameMemento`, `Game.Undo()`, `IGameService.UndoAsync` | `ComputerModeIntegrationTests`, `GameServiceTests.UndoAsync_ComputerMode_UndoesBothMoves` | Unit / Application Test | Domain & Application Complete; API/UI Deferred (P010/P011) |
| **FR-11** (Scoreboard) | ADR-007 | `Scoreboard` Aggregate, `IScoreboardRepository`, `InMemoryScoreboardRepository`, `IScoreboardService` | `ScoreboardTests`, `GameCompletedEventHandlerTests`, `ScoreboardServiceTests` | Unit / Application Test | Domain & Application Complete; API and UI Presentation Deferred (P010/P011) |
| **FR-12** (Reset Game) | A-002, P008.2, P009.1.1 | `Game.Reset()`, `IGameService.ResetGameAsync` | `GameResetTests`, `GameServiceTests.ResetGameAsync_PreservesGameIdAndResetsBoard` | Unit / Application Test | Domain & Application Complete; API Endpoint and UI Action Deferred (P010/P011) |
| **FR-13** (Reset Scoreboard)| ADR-007, P008.2 | `Scoreboard.Reset()`, `IScoreboardService.ResetScoreboardAsync` | `ScoreboardTests`, `ScoreboardServiceTests.ResetScoreboardAsync_ResetsCounters` | Unit / Application Test | Domain & Application Complete; API Endpoint and UI Action Deferred (P010/P011) |
| **FR-14** (Computer Strategy)| ADR-005, Strategy | `IComputerMoveStrategy`, `BasicComputerMoveStrategy`, `IGameService.MakeMoveAsync` | `ComputerStrategyTests`, `GameServiceTests.MakeMoveAsync_ComputerMode_ExecutesBothHumanAndComputerMoves` | Unit / Application Test | Domain & Application Complete; API/UI Deferred (P010/P011) |
| **FR-15** (REST API) | ADR-001, REST DTOs | `GamesController`, `ScoreboardController` | API Integration Tests (`WebApplicationFactory`) | Integration Test | Host Scaffolded (/health verified); Endpoints Deferred to P010 |
| **FR-16** (Backend Ownership)| ADR-001 | ASP.NET Core API Backend | End-to-End verification of state authority | E2E Test | Baseline Planned; Enforced in Domain & Application |
| **FR-17** (Frontend UI) | Layered Arch | Angular App (`GameBoard`, `Scoreboard`, etc.) | Jasmine/Karma / Vitest Component Tests | Unit / E2E Test | App Scaffolded (Build & Test OK); Components Deferred to P011 |
| **FR-18** (Testing) | Test Strategy | `TicTacToe.Tests`, Frontend specs | Test runner output, coverage reports | CI / Test Execution Log | 189 Backend Tests + 2 Frontend Tests Passing |
| **FR-19** (AI Governance) | ADR-008, Governance | `docs/ai/` audit files, commit history | Audit ledger & Git log inspection | Documentation | Active (P001–P008 Closed, P009 Review Ready) |
