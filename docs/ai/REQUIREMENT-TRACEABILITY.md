...
|---|---|---|---|---|---|
| **FR-01** (Create Game) | ADR-001, ADR-002 | `Game` Aggregate, `IGameService.CreateGameAsync`, `GamesController.CreateGame` | `Create_InitializesGameInStandardInitialState`, `GamesApiTests.CreateGame_Returns201Created_WithLocationHeaderAndGameState` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Deferred (P011) |
| **FR-02** (Board 0..8) | A-001, ADR-001 | `Board` Entity, `CellIndex` Value Object, `GameStateDto`, `GamesController` | `BoardTests`, `CellIndexTests`, `ApiValidationTests.MakeMove_UnknownPropertyRowColumn_Returns400BadRequest` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Rendering Deferred (P011) |
| **FR-03** (Turns) | ADR-001, ADR-003 | `Game.MakeMove()`, `IGameService.MakeMoveAsync`, `GamesController.MakeMove` | `MakeMove_ValidMove_PlacesMarkAppendsHistoryAndAlternatesTurn`, `GamesApiTests.MakeMove_InvalidTurn_Returns409Conflict` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Deferred (P011) |
| **FR-04** (Win Detection) | ADR-001, ADR-007 | `WinDetector` Domain Service, `Game.MakeMove`, `GamesController.MakeMove` | `WinDetectorTests`, `GamesApiTests.MakeMove_PlayerX_WinsGame_Returns200OK_WithWinningStateAndUpdatesScoreboard` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Deferred (P011) |
| **FR-05** (Draw Detection) | ADR-001, ADR-007 | `WinDetector` Domain Service, `Game.MakeMove`, `GamesController.MakeMove` | `MakeMove_StandardDraw`, `GamesApiTests.MakeMove_FullBoardDraw_Returns200OK_WithDrawStateAndUpdatesScoreboard` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Deferred (P011) |
| **FR-06** (Move Validation) | ADR-001, ADR-003 | `Game.MakeMove()` invariants, Domain Exceptions, `ExceptionHandlingMiddleware` | `MakeMove_OccupiedCell`, `ApiValidationTests.MakeMove_NegativeCellIndex_Returns400BadRequest` | Unit / API Integration Test | Implemented & Integration Verified (400/409 ProblemDetails); API Exposed; UI Deferred (P011) |
| **FR-07** (Move History) | A-001, ADR-001 | `Move` Value Object, `Game.MoveHistory`, `MoveDto`, `GameStateDto` | `MakeMove_SequentialValidMoves`, `ApiSerializationTests.MoveHistory_SerializesPlayerAsString_AndCellIndexAsNumber` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Presentation Deferred (P011) |
| **FR-08** (Undo Availability)| ADR-004 (Option A) | `Game.CanUndo`, `Game.Undo()`, `GamesController.Undo` | `GameUndoTests`, `GamesApiTests.Undo_WhenNoMoves_Returns409Conflict_WithProblemDetails` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Deferred (P011) |
| **FR-09** (Two Player Undo) | ADR-004, Memento | `GameMemento`, `Game.Undo()`, `GamesController.Undo` | `GameUndoTests`, `GamesApiTests.Undo_TwoPlayerMode_RevertsLastMove` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Deferred (P011) |
| **FR-10** (Computer Undo) | ADR-004, Memento | `GameMemento`, `Game.Undo()`, `GamesController.Undo` | `ComputerModeIntegrationTests`, `GamesApiTests.Undo_ComputerMode_RevertsBothHumanAndComputerMoves` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Deferred (P011) |
| **FR-11** (Scoreboard) | ADR-007 | `Scoreboard` Aggregate, `IScoreboardRepository`, `ScoreboardController` | `ScoreboardTests`, `ScoreboardApiTests.GetScoreboard_Returns200OK_WithCurrentScores` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Presentation Deferred (P011) |
| **FR-12** (Reset Game) | A-002, P008.2, P009.1.1 | `Game.Reset()`, `IGameService.ResetGameAsync`, `GamesController.ResetGame` | `GameResetTests`, `GamesApiTests.ResetGame_PreservesGameId_Returns200OK_WithFreshPlayState` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Action Deferred (P011) |
| **FR-13** (Reset Scoreboard)| ADR-007, P008.2 | `Scoreboard.Reset()`, `ScoreboardController.ResetScoreboard` | `ScoreboardTests`, `ScoreboardApiTests.ResetScoreboard_Returns200OK_WithZeroedScores` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Action Deferred (P011) |
| **FR-14** (Computer Strategy)| ADR-005, Strategy | `IComputerMoveStrategy`, `BasicComputerMoveStrategy`, `GamesController.MakeMove` | `ComputerStrategyTests`, `GamesApiTests.MakeMove_InComputerMode_Returns200OK_WithBothMovesExecuted` | Unit / API Integration Test | Implemented & Integration Verified; API Exposed; UI Deferred (P011) |
...
| **FR-17** (Frontend UI) | Layered Arch | Angular App (`GameBoard`, `Scoreboard`, etc.) | Jasmine/Karma / Vitest Component Tests | Unit / E2E Test | Scaffolded (Build & Test OK); Components Deferred to P011 |
| **FR-18** (Testing) | Test Strategy | `TicTacToe.Tests`, Frontend specs | Test runner output, coverage reports | CI / Test Execution Log | Implemented & Verified: 233 Backend Tests + 2 Frontend Tests Passing (0 Warnings, 0 Errors) |
| **FR-19** (AI Governance) | ADR-008, Governance | `docs/ai/` audit files, commit history | Audit ledger & Git log inspection | Documentation | Active Governance (P001–P009 Closed, P010 Audit Complete) |
